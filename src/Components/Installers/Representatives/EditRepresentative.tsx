...
const EditRepresentative = () => {
	...
	const handleUpdate = async (e: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (data.password) formData.append("newPassword", data.password ?? "");

			await updateDetailsOfInstaller(data._id || "", formData);

			toast.success("Representative updated successfully!");
			navigate(allRoutes.VIEW_REPRESENTATIVE.replace(":id", id || ""));
		} catch (error: any) {
	...
	return (
		<PageLayout
			loading={loading}
			// backButtonPath={
			// 	isAdminLoggedIn() || isOfficeManagerLoggedIn() || isManagerLoggedIn()
			// 		? allRoutes.VIEW_REPRESENTATIVE.replace(":id", id || "")
			// 		: allRoutes.DASHBOARD
			// }
		>
			<Typography variant="h5">Edit Representative</Typography>
			<Typography fontSize={15} mt={10}>
				Update photo and personal details of Representative
			</Typography>
