...
const tableHeaders = customerTableHeaders.filter(
  (item) => !["Date", "Address", "Report"].includes(item.text)
);
...
const DashboardData = ({
  ...
  const displayFilters =
    [roles.SUPER_ADMIN, roles.ADMIN_MANGER, roles.DIRECTOR, roles.PSL].includes(
      role ?? ""
    ) || !id;
          ...
          {displayFilters && (
            <DashboardCard delay={0.025} disableHoverEffect>
              ...
              >
                ...
                <FiltersPopover
                  filters={filters}
                  updateFilters={(updated) => setFilters(updated)}
                />
              </Box>
            </DashboardCard>
          )}
