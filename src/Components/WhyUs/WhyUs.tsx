...
const WhyUs = () => {
  ...
  const getDataOfDashboard = async () => {
    ...
    try {
      ...
      const { data: verification } = await verifyAddress(
        user.address || "1939 Irene St, West Covina, CA 91792, USA"
      );
      ...
      <Box px={isSmallScreen ? 32 : 80}>
        {displaySummary ? (
          ...
          <>
            ...
            <YourSolar
              id={modules.YOUR_SOLAR.id}
              data={data.solarSpecs}
              streetViewImage={streetViewImage}
            />
            <FinancialImpact
