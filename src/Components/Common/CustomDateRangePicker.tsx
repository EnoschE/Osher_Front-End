...
export const formatDatesForDisplay = (min: string, max: string) => {
  return `${moment(min, 'MM-DD-YYYY"').format("ll")} to ${moment(
    max,
    'MM-DD-YYYY"'
  ).format("ll")}`;
};
...
const CustomDateRangePicker = ({
  ...
  useEffect(() => {
    if (open) {
      setSelectedDates([
        {
          startDate: minDate
            ? new Date(moment(minDate, "MM-DD-YYYY").format("ll"))
            : new Date(),
          endDate: maxDate
            ? new Date(moment(maxDate, "MM-DD-YYYY").format("ll"))
            : new Date(),
          key: "selection",
          color: colors.primary,
        },
  ...
  return (
    <>
      ...
      >
        {minDate && maxDate
          ? formatDatesForDisplay(minDate, maxDate)
          : "Custom Date Range"}
      </Button>
